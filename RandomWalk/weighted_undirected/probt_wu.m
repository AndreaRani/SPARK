function P= probt_wu(mat, alpha)

% Given a weighted undirected graph G(V, E), its probability transition matrix
...
pinvstr= pinv(diag(str)); % out-strength pseudo inverse matrix

P= alpha.*((pinvstr*mat)+ (1/n).*(a*e'))+ ((1-alpha)*(1/n)).*(e*e');
end