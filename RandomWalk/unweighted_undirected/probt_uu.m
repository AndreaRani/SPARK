function P= probt_uu(mat, alpha)

% Given a binary undirected graph G(V, E), its probability transition matrix
...
pinvdeg= pinv(diag(deg)); % out-degree pseudo inverse matrix

P= alpha.*((pinvdeg*mat)+ (1/n).*(a*e'))+ ((1-alpha)*(1/n)).*(e*e');
end